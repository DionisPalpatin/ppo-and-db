...
func (nr *NoteRepository) GetAllPublicNotes() ([]*models.Note, *bl.MyError) {
	nr.MyLogger.WriteLog("GetAllPublicNotes is called (Repo)", slog.LevelInfo, nil)
...
func (nr *NoteRepository) AddNote(note *models.Note) (int, *bl.MyError) {
	...
	err := addOrUpdateNoteInfo(note, db, ctx, query, true)
	if err != nil {
		var resState *bl.MyError
...
func (nr *NoteRepository) UpdateNoteInfo(note *models.Note) *bl.MyError {
	...
	err := addOrUpdateNoteInfo(note, db, ctx, query, false)
	if err != nil {
		var resState *bl.MyError

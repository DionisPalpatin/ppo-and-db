...
import mylogger "github.com/DionisPalpatin/ppo-and-db/application/internal/logger"
...
func NewCollectionService(icr ICollectionRepository, logger *mylogger.MyLogger) ICollectionService {
	return &CollectionService{icr: icr, logger: logger}
}
...
func (cs *CollectionService) DeleteCollection(id int, user *models.User) *MyError {
	...
	if col.OwnerID != user.Id && user.Role != Admin {
		myErr = CreateError(ErrAccessDenied, "DeleteCollection", "bl")
		return myErr
	}

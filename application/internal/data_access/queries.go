...
const (
	...
	`
	addUserToTeamQuery = `
	INSERT INTO %s.team_members (team_id, user_id) values
	($1, $2);
	`
	...
	SELECT *
	FROM %s.users u
	JOIN %s.team_members tm ON u.id = tm.user_id
	WHERE tm.team_id = $1;
	`
	getUserTeamQuery = `
	SELECT *
	FROM %s.teams t
	JOIN %s.team_members tm ON t.id = tm.team_id
	WHERE tm.user_id = $1;
	`
)
...
const (
	...
	`
	addNoteTextQuery = `
	INSERT INTO %s.texts (data, file_extension, note_id) VALUES
	($1, $2, $3);
	`
	addNoteImageQuery = `
	INSERT INTO %s.images (data, file_extension, note_id) VALUES
	($1, $2, $3);
	`
	addNoteRawDataQuery = `
	...
	UPDATE %s.texts SET (data, file_extension) = ($1, $2)
	WHERE note_id = $3;
	`
	updateNoteImageContentQuery = `
	UPDATE %s.images SET (data, file_extension) = ($1, $2)
	WHERE note_id = $3;
	`
	updateNoteRawContentQuery = `
	UPDATE %s.raw_datas SET (data, file_extension) = ($1, $2)
	WHERE note_id = $3;
	`
	updateNoteInfoQuery = `
	UPDATE %s.notes SET
...
`
	...
	JOIN %s.notes n ON c.id = n.collection_id
	WHERE n.owner_id = $1;
`
	addCollectionQuery = `

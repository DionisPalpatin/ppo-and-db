...
func (sr *SectionRepository) GetSectionByID(id int) (*models.Section, *bl.MyError) {
	...
	db := sr.DbConfigs.DB
	schemaName := sr.DbConfigs.Name
	query := fmt.Sprintf("SELECT * FROM %s.sections WHERE id = ?", schemaName)
...
func (sr *SectionRepository) GetSectionByTeamName(teamName string) (*models.Section, *bl.MyError) {
	...
	db := sr.DbConfigs.DB
	schemaName := sr.DbConfigs.Name
	query := fmt.Sprintf("SELECT id, creation_date FROM %s.sections s JOIN %s.teams_sections ts ON s.id = ts.section_id WHERE t.name = ?", schemaName, schemaName)
	ctx := context.Background()
...
func (sr *SectionRepository) AddSection(section *models.Section, team *models.Team) *bl.MyError {
	...
	db := sr.DbConfigs.DB
	schemaName := sr.DbConfigs.Name
	query := fmt.Sprintf("INSERT INTO %s.sections (creation_date) VALUES (?)", schemaName)
	query1 := fmt.Sprintf("INSERT INTO %s.teams_sections (team_id, section_id) VALUES (?, ?)", schemaName)
...
func (sr *SectionRepository) DeleteSection(id int) *bl.MyError {
	...
	db := sr.DbConfigs.DB
	schemaName := sr.DbConfigs.Name
	query := fmt.Sprintf("ALTER TABLE %s.sections DELETE WHERE id = ?", schemaName)
	query1 := fmt.Sprintf("ALTER TABLE %s.teams_sections DELETE WHERE section_id = ?", schemaName)
	ctx := context.Background()
...
func (sr *SectionRepository) UpdateSection(section *models.Section) *bl.MyError {
	...
	db := sr.DbConfigs.DB
	schemaName := sr.DbConfigs.Name
	query := fmt.Sprintf("ALTER TABLE %s.sections UPDATE creation_date = ? WHERE id = ?", schemaName)
	ctx := context.Background()
...
func (sr *SectionRepository) GetAllNotesInSection(section *models.Section) ([]*models.Note, *bl.MyError) {
	...
	db := sr.DbConfigs.DB
	schemaName := sr.DbConfigs.Name
	query := fmt.Sprintf("SELECT * FROM %s.notes WHERE section_id = ?", schemaName)
	ctx := context.Background()
...
func (sr *SectionRepository) AddNoteToSection(note *models.Note, section *models.Section) *bl.MyError {
	...
	db := sr.DbConfigs.DB
	schemaName := sr.DbConfigs.Name
	query := fmt.Sprintf("ALTER TABLE %s.notes UPDATE section_id = ? WHERE id = ?", schemaName)
	ctx := context.Background()
...
func (sr *SectionRepository) DeleteNoteFromSection(note *models.Note, section *models.Section) *bl.MyError {
	...
	db := sr.DbConfigs.DB
	schemaName := sr.DbConfigs.Name
	query := fmt.Sprintf("ALTER TABLE %s.notes UPDATE section_id = -1 WHERE id = ? AND section_id = ?", schemaName)
	ctx := context.Background()

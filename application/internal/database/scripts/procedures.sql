...
as $$
begin
	delete from main.note_collections where collection_id in (select id from main.collections where owner_id = deleted_user_id);
...
begin
	...
	from interg_tests.teamss_sections
	where team_id = deleted_team_id;

	update main.notes set section_id = 0
	where section_id = team_sec_id;

	delete from interg_tests.teamss_sections where team_id = deleted_team_id;
...
begin
  ...
  delete from main.note_collections where note_id = deleted_note_id;
